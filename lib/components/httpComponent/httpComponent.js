...
HttpComponent.prototype.init = function () {
    ...
    this.on('connection', function (connection) {
        ...
        self.sessions[connection.id] = connection;

        connection.on('disconnect', function () {
            delete self.sessions[connection.id];
        });
    ...
    this.on('message', function (msg, connection) {
        Logger.info("Request msg from connection [" + connection.id + "] : " + JSON.stringify(msg));
...
HttpComponent.prototype.start = function () {
    this.emit(STARTING);
    Logger.info("HttpComponent " + this.opts.id + " is starting...");

    this.linstener.listen(this.opts.port, this.opts.host);

    Logger.info("HttpComponent " + this.opts.id + " was started.");
    this.emit(STARTED);
}
...
var ProcessMessage = function (server, request, response) {
    var connection = Bearcat.getBean('httpConnection', ReqID++, response, server.opts.procotol || "");
    if (!connection) {
    ...
    request.addListener('end', function () {
        server.protocol.decode(Buffer.concat(bytes), server.opts.procotol || "", function (error, data) {
            if (error) {
