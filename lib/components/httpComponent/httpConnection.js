...
var EventEmitter = require('events').EventEmitter,
    Util = require('util'),
    Logger = require('pomelo-logger').getLogger('zplay-lib', 'httpConnection');
...
Connection.prototype.init = function () {
    ...
    this.on('error', function (error) {
        Logger.warn("Connection " + this.id + " has an error :" + error.stack + "");
    });
    var _this = this;
    this.on('disconnect', function () {
        Logger.info("Connection " + this.id + " disconnected  time-consumed:" + (new Date() - _this.__createTime));
...
Connection.prototype.send = function (msg, status) {
    ...
    Logger.info("Response message to connection [" + this.id + "] " + strBuf.length + " : " + strBuf);
