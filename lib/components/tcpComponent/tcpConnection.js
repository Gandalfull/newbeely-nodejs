 ...
 * @param cb
 */
TcpConnection.prototype.send = function (type, msg, encode, cb) {
    if (this.socket && this.state == Working) {
        var _this = this;
        this.protocol.encode(msg, this.encrypts, function (error, data) {
...
TcpConnection.prototype.kick = function (reson) {
    this.send(0, reson, "utf8", this.disconnect.bind(this));
};
...
function MsgProcesser(connection, chunk) {
    ...
    if (connection.socketState == S_BODY) {
        ...
        connection.reciveSize += chunk.length;
    }
    if (connection.packageBuffer.length == connection.reciveSize) {
        ...
        connection.protocol.decode(message, connection.encrypts, function (error, data) {
            ...
            try {
                connection.emit('message', JSON.parse(data));
            } catch (e) {
                Logger.warn("decode messgae warn:" + e);
            }
        });
